...
for r in range(1, n_row+1):
    for c in range(1, n_col+1):
        ...
        if ne_block_entity == 'W':
            option1_money -= 20000
        south_intersection_money = dp_table[r][c-1]
        option2_money = south_intersection_money
        if nw_block_entity == 'B':
            option2_money += 10000
        if nw_block_entity == 'W':
            option2_money -= 20000
...
if (dp_table[n_row,n_col]) > 0:
    print 'As you can see, you are now rich! :)'
else:
    print 'You are broke. :('

print 'The path to success:'
print choices_table
